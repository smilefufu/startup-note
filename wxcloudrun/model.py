...
class DifyUsers(db.Model):
    ...
    xx_user_name =  db.Column(db.String(100), nullable=True)
    xx_phone_num = db.Column(db.String(100), nullable=True)
    xx_xiaoxi_uuid = db.Column(db.String(100), nullable=True)
    # wx_  代表来自微信平台等数据
    ...
    # 随机生成的注册码
    register_code = db.Column(db.String(100), nullable=True)
    # 其余数据
    created_at = db.Column('createdAt', db.TIMESTAMP, nullable=False, default=datetime.now())
    updated_at = db.Column('updatedAt', db.TIMESTAMP, nullable=False, default=datetime.now())